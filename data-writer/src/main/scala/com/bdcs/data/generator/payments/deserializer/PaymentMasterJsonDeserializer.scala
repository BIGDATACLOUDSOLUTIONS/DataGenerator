...
class PaymentMasterJsonDeserializer extends Deserializer[PaymentMasterJson] {
   ...
   * @return T
   */
  override def deserialize(topic: String, data: Array[Byte]): PaymentMasterJson = {
    try {
      if (data == null) {
      ...
      objectMapper.readValue(new String(data, "UTF-8"), classOf[PaymentMasterJson])
    } catch {
      case e: Exception =>
